...
//             // If using express.json(), body is already parsed
//             const expectedSignature = crypto
...
const paymentController = {
    ...
    cancelSubscription: async (request, response) => {
        try {
            ...
            console.log(error);
            response.status(500).json({
                message: error && error.error && error.error.description ? error.error.description : (error.message || 'Internal server error')
            });
        }
    },
